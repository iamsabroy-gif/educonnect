...
- PostgreSQL 17 via `pg` — same engine locally (Homebrew) and in production (Supabase free tier)
- Tailwind CSS v4
...
`.env.local` points `DATABASE_URL` at the local `educonnect` database. To run locally against the production Supabase database instead, swap in the session-pooler URL (see `.env.example`).
...
Database: Supabase free-tier project `gzuibxcoeevnmnvbezpm` (ap-southeast-1). Schema is already applied as migrations, with RLS enabled deny-all so the auto-generated Supabase REST API cannot touch the tables (the app connects directly over Postgres).

1. Push this repo to GitHub.
2. In Render: **New → Blueprint**, pick the repo (`render.yaml` configures a free Singapore web service).
3. Set the `DATABASE_URL` env var to the Supabase **session pooler** URL
   (Supabase dashboard → Connect → Session pooler; use this, not the direct
   connection, because Render free tier is IPv4-only):
   `postgres://postgres.gzuibxcoeevnmnvbezpm:<DB-PASSWORD>@aws-0-ap-southeast-1.pooler.supabase.com:5432/postgres`
   (Reset the DB password under Project Settings → Database if you don't have it.)
...
- **Supabase:** project pauses after ~7 days with no traffic; unpause from the dashboard. 500 MB database limit — with 5 MB max uploads, keep an eye on usage (`SELECT pg_size_pretty(pg_database_size(current_database()));`).
